...
const RippleCursor = ({ maxSize = 50, duration = 1000, blur = true }) => {
  ...
  return (
    <div
      className="fixed top-0 left-0 w-screen h-screen pointer-events-none overflow-hidden z-[9999]"
      style={{ position: 'fixed' }}
    >
      {ripples.map((ripple) => (
        <div
          key={ripple.id}
          className="rounded-full bg-blue-500 bg-opacity-50 shadow-[0_0_10px_rgba(0,150,255,0.7),0_0_20px_rgba(0,150,255,0.4)] animate-ripple"
          style={{
            position: 'fixed',
